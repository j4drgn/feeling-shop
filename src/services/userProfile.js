...
export class UserProfileService {
  constructor() {
    this.storageKey = 'feeling_shop_user_profile';
    this.interactionsKey = 'feeling_shop_interactions';
  ...
  saveInteractions() {
    try {
      localStorage.setItem(this.interactionsKey, JSON.stringify(this.interactions));
    } catch (error) {
  ...
  // 제품 상호작용 기록
  recordProductInteraction(type, product) {
    const interaction = {
      timestamp: Date.now(),
    ...
    switch (type) {
      case 'view':
        this.interactions.productViews.push(interaction);
        break;
      case 'like':
        this.interactions.likes.push(interaction);
        this.updateCategoryPreference(product.category, 1);
        break;
      case 'purchase':
        this.interactions.purchases.push(interaction);
        this.updateCategoryPreference(product.category, 3);
        break;
    }
  ...
  updateCategoryPreference(category, weight) {
    if (!this.userProfile.preferences.categories[category]) {
      this.userProfile.preferences.categories[category] = 0;
    }
    this.userProfile.preferences.categories[category] += weight;
    this.saveProfile();
  }
